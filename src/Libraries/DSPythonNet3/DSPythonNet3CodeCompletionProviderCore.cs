...
namespace DSPythonNet3
{
    internal class DSPythonNet3CodeCompletionProviderCore : PythonCodeCompletionProviderCommon, ILogSource, IDisposable
    {
        ...
        private static string globalScopeName = "pythonNet3CompletionGlobal";
...
");
        ...
        {
            DSPythonNet3.PythonNet3Evaluator.InstallPython();
            ...
            using (Py.GIL())
            {
                ...
                return PythonNet3Evaluator.OutputMarshaler.Marshal(result);
            }
        }
        ...
        /// This method calls into Python objects and thus needs to be wrapped in a GIL block.
        /// </summary>
        ...
        private IEnumerable<Tuple<string, string, bool, ExternalCodeCompletionType>> EnumerateMembers(object pyObject, string name)
        {
            ...
            try
            {
                // Useful when parsing CLR namespaces (ex. System.Collections.)
            ...
            foreach (var member in d.Dir())
            {
                try
                {
                    ...
                    {//Ignore python specific members when dealing with wrapped .NET objects
                        continue;
                    }
        ...
        {
            if (engineName == PythonEngineManager.PythonNet3EngineName)
            {
                return true;
            }
            return false;
